...
function show_info($atts, $content = null) {
    ...
    return '<p class="message ' . $alert_classes['info'] . '">'.do_shortcode($content).'<button type="button" class="close" aria-hidden="true">&times;</button></p>';
}
...
function show_tip($atts, $content = null) {
	global $alert_classes;
    return '<p class="message ' . $alert_classes['warning'] . '">'.do_shortcode($content).'<button type="button" class="close" aria-hidden="true">&times;</button></p>';
}
...
function show_error($atts, $content = null) {
	global $alert_classes;
    return '<p class="message ' . $alert_classes['danger'] . '">'.do_shortcode($content).'<button type="button" class="close" aria-hidden="true">&times;</button></p>';
}
...
function show_success($atts, $content = null) {
	global $alert_classes;
    return '<p class="message ' . $alert_classes['success'] . '">'.do_shortcode($content).'<button type="button" class="close" aria-hidden="true">&times;</button></p>';
}
...
if (!function_exists('show_tabs')) {
    function show_tabs($atts, $content = null)
    {
        ...
        $html = '<div class="clearfix"><ul class="tabs-nav ' . $layout . ' clearfix">';
        foreach ($all_title as $title) {
            $html .= '<li>' . $title . '</li>';
        }
        $html .= '</ul><div class="tabs-container ' . $layout . '">' . do_shortcode($content) . '</div></div>';
        return $html;
    }
}
                ...
                <div class="<?php bc('12','12','12',''); ?>">
                    <div class="appointment-form clearfix appointment-shortcode">
                        <form class="row" id="appointment_form_three" action="<?php echo esc_url(admin_url('admin-ajax.php')); ?>" method="post">
                            ...
                            <div class=" <?php bc_all( '12' ); ?>">
		                        <?php
		                        if ( '1' == $theme_options['display_appointment_recaptcha'] ) {
			                        /* Display recaptcha if enabled and configured from theme options */
			                        get_template_part( INSPIRY_COMMON . '/recaptcha/custom-recaptcha' );
		                        }
		                        ?>
                            </div>
                            ...
                            <div class=" <?php bc_all('12'); ?>">
                                ...
                                <img src="<?php echo INSPIRY_ASSETS_DIR_URI; ?>/images/loader.gif" id="appointment-loader" alt="<?php _e('Loading...', 'framework'); ?>">
...
if (!function_exists( 'inspiry_get_doctor' ) ) {
	function inspiry_get_doctor( $atts, $content = null ) {
		...
		if ( $doctor_query->have_posts() ) {
			...
			while ( $doctor_query->have_posts() ) {
				...
				if ( 'list' == $atts['style'] ) :
                    ...
                    <div class="doctor-shortcode-post doctors-posts doctors-page style-horizontal <?php echo $atts['align']; ?>">
                        <article class="common-doctor row">
                            <div class="col-sm-6">
								<?php inspiry_standard_thumbnail( 'gallery-post-single' ); ?>
                            </div>
                            <div class="col-sm-6">
					...
					?>
                    <div class="doctor-shortcode-post doctors-posts doctors-page style-vertical clearfix align<?php echo $atts['align']; ?>">
                        <article class="common-doctor text-center">
							<?php inspiry_standard_thumbnail( 'gallery-post-single' ); ?>
                            <div class="text-content">
                                <h5 class="entry-title">
                                    <a href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
			...
			$msg = __( 'No Doctors found!', 'framework' );

			if ( $id ) {
				$msg .= sprintf( __( ' with ID: %s', 'framework' ), $id );
			} else if ( $name ) {
				$msg .= sprintf( __( ' with Name: %s', 'framework' ), $name );
			}
	...
	add_shortcode( 'doctor', 'inspiry_get_doctor' );
}
                ...
                while ($blog_query->have_posts()) {
                    ...
                    ?>">
                        <article class="common-blog-post hentry clearfix">
                            <?php get_template_part(INSPIRY_PARTIALS . "/blog/post/formats/grid/$format"); ?>
